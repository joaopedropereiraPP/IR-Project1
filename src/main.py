...
class Main:
    ...
    def check_arguments(self, parser, args):
        ...
        if args.mode == 'indexer':
            ...
            # method
            if args.method:
                if args.method == 'bm25' or args.method == 'lnc.ltc' \
                        or args.method == 'raw':
            ...
            if args.data_path:
                ...
                if not path.exists(self.data_path) \
                        or not self.data_path.endswith('.gz'):
                    print('File does not exist or does not have the correct '
                          + 'extension! ')
            ...
            # if stopwords are disabled but a stopwords path is still defined
            # by the user
            ...
            if args.data:
                ...
                parser.error(
                    'Search mode requires --data and --search_type.')
                sys.exit()
            ...
            if not args.dump_file and not args.cmd_results:
                parser.error(
                    'Search type requires at least one of two options: '
                    + '--dump_file / --cmd_results')
                sys.exit()
    ...
    def main(self):
        ...
        if self.mode == 'indexer':
            ...
            query = Query(
                data_path=self.data, dump_results_file=self.dump_results_file,
